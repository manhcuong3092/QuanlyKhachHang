<?php
  ...
  class Database {
    ...
    {
      $this->link = mysqli_connect("localhost:3307", "cuong", "123456", "quanlykhachhang") or die();
    ...
    public function deleteWhere($table, array $data) {
      foreach($data as $id) {
        $id = intval($id);
        $sql = 'DELETE FROM {$table} WHERE id = $id ';
        mysqli_query($this->link, $sql) or die("Lỗi truy vấn delete " . mysqli_error($this->link));
    ...
    public function deletesql($table, $sql) {
      ...
      mysqli_query($this->link, $sql) or die("Lỗi truy vấn delete sql ---" . mysqli_error($this->link));
      return mysqli_affected_rows($this->link);
    }
